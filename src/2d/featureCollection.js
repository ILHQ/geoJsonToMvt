...
function normalizeProperties(properties, featureIndex) {
    ...
    const clonedProperties = cloneJsonValue(
        properties,
        `第 ${featureIndex + 1} 个要素的 properties`
    ) || {};
    const normalizedProperties = {};

    Object.entries(clonedProperties).forEach(([key, value]) => {
 ...
 * 复制二维坐标。
 */
function clone2DPosition(position) {
...
function normalizePosition(position, context, state) {
    ...
    return clone2DPosition(position);
}
...
/**
 * 确保 Polygon / MultiPolygon 的环首尾闭合。
 */
...
/**
 * 归一化 FeatureCollection，输出二维几何和可安全写入 MVT 的属性。
 */
